...
export default function Subscribe({ userId }: SubscribeProps) {
  ...
  return (
    <button
      onClick={toggleBlock}
      className={`rounded-full px-4 py-2 font-medium text-bg transition-colors ${follow ? "bg-red hover:bg-red-light" : "bg-primary hover:bg-primary-hover"} ${blocked ? "cursor-not-allowed" : "cursor-pointer"}`}
    >
